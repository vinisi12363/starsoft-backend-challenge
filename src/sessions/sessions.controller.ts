...
export class SessionsController {
    ...
    @ApiOperation({ 
        ...
        description: 'Ao criar uma sessão, o sistema automaticamente gera o estado inicial (AVAILABLE) de todos os assentos da sala vinculada.' 
    })
    ...
    @ApiOperation({ 
        ...
        description: 'Retorna todos os assentos da sessão com seus respectivos status (AVAILABLE, RESERVED, SOLD).' 
    })
    @ApiParam({ name: 'id', description: 'ID da Sessão (UUID)' })
    ...
    async getSeatAvailability(@Param('id', ParseUUIDPipe) id: string) {
        // Alinhado com o método getSessionMap da nossa Service refatorada
        return this.sessionsService.getSessionMap(id);
    }
}